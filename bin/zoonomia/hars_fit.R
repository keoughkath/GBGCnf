...
#=========================================================
	...
	res.null        = phyloFit(     ali,
                                        ...
                                        quiet=TRUE,
                                        bound=n.bound)
        bl.null         = summary.tree(res.null$tree)[ which(summary.tree(res.null$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
	...
	res.s           = phyloFit(     ali,
                                        ...
                                        quiet=TRUE,
                                        bound=a.bound)
        bl.s            = summary.tree(res.s$tree)[ which(summary.tree(res.s$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
	...
	bl.s.r          = summary.tree(res.s.r$tree)[ which(summary.tree(res.s.r$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
	...
	res.s.p         = phyloFit(     ali,
                                        ...
                                        no.opt=no.opt.str,
                                        quiet=TRUE)
        bl.s.p          = summary.tree(res.s.p$tree)[ which(summary.tree(res.s.p$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
	...
	#=========================
	 ...
	 res.b           = phyloFit(    ali,
                                        ...
                                        quiet=TRUE,
                                        bound=b.bound)
        bl.b            = summary.tree(res.b$tree)[ which(summary.tree(res.b$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
        ...
        res.sb          = phyloFit(     ali,
                                        ...
                                        quiet=TRUE,
                                        bound=ab.bound)
        bl.sb           = summary.tree(res.sb$tree)[ which(summary.tree(res.sb$tree)[,"name"]=="Homo_sapiens")  ,"tparent"]
...
#============================
        ...
        res.neutral.t           = phyloFit(     ali                             ,
                                                init.mod=mod                    ,
                                                scale.only=TRUE                 ,
                                                scale.subtree="Homo_sapiens:loss"       ,
                                                no.opt="ratematrix"             ,
                                                quiet=TRUE                      )
...
fitMixGBGC2 <- function(        modB,
        ...
        modB.mix = tmp[[ind]]
        modB.mix$ls.model$defn = "Homo_sapiens#1:bgc[0,1000]"
        bgc                  = phyloFit(ali, init.mod=modB.mix,
                                                no.opt=c( "ratematrix","branches","backgd"))
        ...
        modAB.mix   = add.ls.mod(  modAB.mix, branch="Homo_sapiens", category=1,
                                 separate.params="sel[0,1000]", sel=0)
        modAB.mix  =  add.ls.mod( modAB.mix, branch="Homo_sapiens", category=2,
        ...
        for(fac in facs){
                ...
                mod.b                   = add.ls.mod(mod.b, branch="Homo_sapiens", category=2, separate.params="bgc[1,100]", bgc=fac*modB$ls.model$bgc)
                ...
                mod.ab                  = add.ls.mod(  mod.ab, branch="Homo_sapiens", category=1,
                ...
                mod.ab                  =  add.ls.mod( mod.ab, branch="Homo_sapiens", category=2,
...
#=========================================
	...
	mod$rate.matrix         = apply.bgc.sel(mod$rate.matrix,s=mod$selection)
        mod                     = add.ls.mod(mod, branch="Homo_sapiens", separate.params="sel[,1000]",sel=hg.sel)
        return(mod)
}
...
#=========================================================
        ...
        # add gBGC to the human branch in the 2nd category only.  It seems to be necessary
        # to set the gBGC parameter to something > 0 or else it often gets stuck at 0
        mod                     = add.ls.mod(mod, branch="Homo_sapiens", category=2, separate.params="bgc[0,1000]", bgc=IN.bgc)
        return(mod)
}
...
#======================================================
        ...
        mod                     = add.ls.mod(  mod, branch="Homo_sapiens", category=1,
                                                separate.params="sel[0,1000]", sel=IN.sel)
        # The sel#1 notation in the following command is currently undocumented, but implies
        # that the value is shared with selection from category 1
        mod                     = add.ls.mod(  mod, branch="Homo_sapiens", category=2,
